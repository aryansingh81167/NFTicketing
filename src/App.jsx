  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (window.ethereum) {
        ...
        currentContract = new ethers.Contract(CONTRACT_ADDRESS, TICKET_NFT_ABI, currentProvider);
      } else {
        // Fallback to default rpc for sepola or just skip
        const rpcProvider = new ethers.JsonRpcProvider("https://rpc.sepolia.org");
        currentContract = new ethers.Contract(CONTRACT_ADDRESS, TICKET_NFT_ABI, rpcProvider);
      }
              ...
              {verifyResult !== null && (
                ...
                >
                  <div className={`p-4 rounded-xl border flex items-start gap-3 ${
                    verifyResult ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-rose-500/10 border-rose-500/30'
                  }`}>
                    {verifyResult ? (
